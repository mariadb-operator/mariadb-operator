...
func (r *GaleraReconciler) reconcileRecovery(ctx context.Context, mariadb *mariadbv1alpha1.MariaDB,
	...
	if rs.bootstrapTimeout(mariadb) {
		...
		r.recorder.Eventf(mariadb, nil, corev1.EventTypeWarning, mariadbv1alpha1.ReasonGaleraClusterBootstrapTimeout,
			mariadbv1alpha1.ReasonGaleraClusterBootstrapTimeout, "Galera cluster bootstrap timed out")
		...
		g.Go(func() error {
			...
			err = wait.PollWithMariaDB(recoveryCtx, mariadbKey, r.Client, stateLogger, func(ctx context.Context) error {
				...
				r.recorder.Eventf(mariadb, nil, corev1.EventTypeNormal, mariadbv1alpha1.ReasonGaleraPodStateFetched,
					mariadbv1alpha1.ReasonGaleraPodStateFetched, "Galera state fetched in Pod '%s'", pod.Name)
			...
			if err = wait.PollWithMariaDB(recoveryCtx, ctrlclient.ObjectKeyFromObject(mariadb), r.Client, recoveryLogger,
				func(ctx context.Context) error {
					...
					r.recorder.Eventf(mariadb, nil, corev1.EventTypeNormal, mariadbv1alpha1.ReasonGaleraPodRecovered,
						mariadbv1alpha1.ReasonGaleraPodRecovered, "Recovered Galera sequence in Pod '%s'", pod.Name)
