...
- **Single multi-command binary**: `cmd/controller` root command *is* the operator; `webhook` and `cert-controller` are subcommands, `backup` nests a `restore` subcommand, `agent` has `replication`/`galera`/`standalone` subcommands and `init` has `replication`/`galera`. All topologies run the agent sidecar (standalone included — configured via `spec.agent`); only HA topologies have an init container.
...
- **Webhooks don't default**: defaulting happens in the controller's `Spec` phase (`setSpecDefaults`) and API helper methods, not in mutating webhooks (there are none). Corollary: the Spec phase **persists** defaults into `spec` (e.g. `bootstrapFrom` gets `s3`/`volume`/`volumeSnapshotRef` filled in from the referenced backup), and that patch goes through the validation webhook like any user update — so `Validate()` logic must accept the *post-defaulting* shape of the spec, not just what users write by hand. Test entries commented "This case happens when setting defaults…" encode this contract; don't delete them to make a stricter validation pass.

### PITR (binlog archival) across topologies

- **Galera nodes share GTIDs but have different binlog files.** Archival must be pinned to a single stable pod (pod index 0 for Galera/standalone; `status.currentPrimary` for replication — see `shouldArchiveBinlogs` in `pkg/binlog/archiver.go`). Never archive from a floating primary in Galera: the timeline breaks on failover and recovery becomes impossible. PITR correctness depends on a **gapless** archive from a **single** binlog sequence.
- **`gtid_domain_id` and `wsrep_gtid_domain_id` must never be equal on a node.** `pkg/galera/config/config.go` derives per-pod `gtid_domain_id` from the pod index with offsets (`gtidDomainIDOffset` for multi-cluster, `pitrGtidDomainIDOffset` for PITR) to guarantee this. Multi-cluster GTID settings take precedence over PITR when both are enabled.
- **Pod-index-dependent Galera config interacts with the update-annotation hash**: `getUpdateAnnotations` (`internal/controller/mariadb_controller_update.go`) renders the Galera config with a synthetic pod name to compute a per-StatefulSet hash. Any config content that varies by pod index must keep that render stable (index 0), or every reconcile flaps the hash. Conversely, changing what the config renders (e.g. enabling PITR on an existing Galera cluster) changes the hash and triggers a rolling update — deliberate, but verify it only fires for the intended instances.
- **During PITR binlog replay** `wsrep_on=OFF` is set on the replaying node; agent probes special-case this window (`pkg/agent/handler/galera/probe.go`). Be careful with readiness there: a Ready pod enters Service endpoints while data is mid-restore.
