...
| `semiSyncWaitNoSlave` _boolean_ | SemiSyncWaitNoSlave determines whether the primary waits for the semi-synchronous ACK timeout to expire when it has<br />no semi-synchronous replicas connected. Since the operator configures every node as a semi-synchronous primary, replicas<br />with log_slave_updates enabled also hit this wait when applying transactions. Disabling it makes them fall back to<br />asynchronous commit immediately instead of stalling for the whole semiSyncAckTimeout.<br />It requires semi-synchronous replication to be enabled.<br />It is enabled by default by the server.<br />See: https://mariadb.com/docs/server/ha-and-performance/standard-replication/semisynchronous-replication#rpl_semi_sync_master_wait_no_slave |  |  |
